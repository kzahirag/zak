...
(function($){  
	...
	$.fn.zakmenu = function (clickEvent, param, myReplaces, effect) {  
		...
		if ( clickEvent == "rightClick" || clickEvent == "both" )
		{
    		...
    		document.body.oncontextmenu = function() {
                if ($("div[id^=zakmenu_main]").length) return false;
            }			
		}
		...
		$(document).click(function(event) { if (event.button!=2) $("div[id^=zakmenu]").remove(); });
		...
		function menu(id,param,myReplaces,el,effect) {
			...
			m.className = "zakmenu";	m.id = "zakmenu_"+id;
			$(m).css({display:'none'});
			...
			function putItem(n) {
				...
				switch (n.type)
				{
					case "sub":
						...
						$(item).click(function() {	 	
							if ($("#zakmenu_"+id+"_sub").length > 0) {
								$("div[id^=zakmenu_"+id+"_sub]").remove();
							}
							else {
					...
					default:
					  $(item).hover(function() { $("div[id^=zakmenu_"+id+"_sub]").remove(); });
					 // console.log(n.customClass);
						//if((n.customClass).isHeader('nav-header')){
							//item.className = "jj_menu_item";
							//item.className = "nav-header";
						//}
					break;
				}
				...
				if (n.customClass && n.customClass.length>0) {
					...
					{
						jQuery(a).addClass(n.customClass);
					}
				}
				
				$(a).html('<i class="icon-wrench"></i> '+uReplace(n.title));
		...
		String.prototype.isHeader = function(it) { return this.indexOf(it) != -1; };
		
		function initModal(){
			$("div[id^=zakmenu]").remove(); 
